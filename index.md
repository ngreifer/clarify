...
describing the package (also available at
[`vignette("clarify")`](articles/clarify.md)). *clarify* was designed to
replicate and expand on functionality previously provided by the *Zelig*
package.
...
other examples. For a complete vignette, see
...
primary functions of *clarify* are [`sim()`](reference/sim.md),
[`sim_apply()`](reference/sim_apply.md),
[`summary()`](https://rdrr.io/r/base/summary.html), and
[`plot()`](https://rdrr.io/r/graphics/plot.default.html). These work
together to create a simple workflow for simulation-based inference.

- [`sim()`](reference/sim.md) simulates model parameters from a fitted
  model
- [`sim_apply()`](reference/sim_apply.md) applies an estimator to the
  simulated coefficients, or to the original object but with the new
  coefficients inserted
- [`summary()`](https://rdrr.io/r/base/summary.html) produces confidence
  intervals and p-values for the resulting estimates
- [`plot()`](https://rdrr.io/r/graphics/plot.default.html) produces
  plots of the simulated sampling distribution of the resulting
  estimates

There are also some wrappers for [`sim_apply()`](reference/sim_apply.md)
...
[`sim_adrf()`](reference/sim_adrf.md) computes average dose-response
functions. *clarify* also offers support for models fit to multiply
imputed data with the [`misim()`](reference/misim.md) function.

In the example above, we used [`sim_ame()`](reference/sim_ame.md) to
compute the ATT, but we could have also done so manually using
...
[`transform()`](https://rdrr.io/r/base/transform.html) on the
...
of *Zelig*’s `setx()` and `setx1()` functions, using
...
See [`vignette("Zelig", package = "clarify")`](articles/Zelig.md) for
more examples of translating a *Zelig*-based workflow into one that uses
*clarify* to estimate the same quantities of interest.
...
of models fit to multiply imputed data. See
[`vignette("clarify")`](articles/clarify.md) for more details.
