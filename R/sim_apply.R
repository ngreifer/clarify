...
#' @param verbose `logical`; whether to display a text progress bar indicating progress and estimated time remaining for the procedure. Default is `TRUE` for interactive sessions and `FALSE` otherwise.
...
#'   [`transform()`][transform.clarify_est].
#'
#' @returns
...
sim_apply <- function(sim,
  ...
  if (missing.fun || is_null(FUN)) {
    if (verbose && missing.fun) {
      arg::wrn("{.arg FUN} not supplied; returning simulated coefficients")
    }
  ...
  if (inherits(sim, "clarify_misim")) {
    ...
    tryCatch(
      {
        test <- lapply(seq_len(nimp), function(i) {
          apply_FUN(fit = sim$fit, coefs = test.coefs, imp = i, ...)
        })
      },
      error = function(e) {
        arg::err("{.arg FUN} failed to run on an initial check with the following error:\f{conditionMessage(e)}")
      }
    )
    ...
    tryCatch(
      {
        test <- apply_FUN(fit = sim$fit, coefs = sim$coefs, ...)
      },
      error = function(e) {
        arg::err("{.arg FUN} failed to run on an initial check with the following error:\f{conditionMessage(e)}")
      }
