...
#' @param x1 a data.frame or named list of the value each predictor should take to compute the first difference from the predictor combination specified in `x`. `x1` can only identify a single unit. See Details.
...
#' * [plot.clarify_setx()] for plotting the output of a call to `sim_setx()`.
#' * [summary.clarify_est()] for computing p-values and confidence intervals for the estimated quantities.
#' * [marginaleffects::predictions()] and [marginaleffects::comparisons()] for delta method-based implementations of computing predicted values and first differences.
#'
...
sim_setx <- function(sim,
  ...
  if (fd) {
    if (is_null(.attr(newdata, "set_preds"))) {
      arg::err("when {.arg x1} is specified, {.arg x} must be specified")
    }

    if (nrow(newdata) != 1L) {
      arg::err("when {.arg x1} is specified, {.arg x} must identify a single reference unit")
    }

    newdata_x1 <- process_x(x1, dat, "x1")

    if (nrow(newdata_x1) != 1L) {
      arg::err("{.arg x1} must identify a single unit")
    }

    if (!setequal(.attr(newdata, "set_preds"), .attr(newdata_x1, "set_preds"))) {
      arg::err("when {.arg x1} is specified, the same variables must be specified in {.arg x} and {.arg x1}")
    }

    if (all_apply(names(newdata), function(i) identical(newdata[[i]], newdata_x1[[i]]))) {
      arg::err("{.arg x} and {.arg x1} must be different")
    }
...
process_x <- function(x, dat, arg_name) {
  ...
  if (is_null(x)) {
    ...
    if (is_null(set_preds)) {
      arg::wrn("the data frame supplied to {.arg {arg_name}} does not contain any variables that correspond to variables used in the original model")
    }
  }
  else if (!is.list(x) || is_null(names(x)) || !all(nzchar(names(x))) ||
           !all_apply(x, rlang::is_atomic)) {
    arg::err("the argument to {.arg {arg_name}} must be a grid data frame or a named list of values for variables to be set to")
  }
    ...
    arg::err("{cli::qty(vars_not_in_model)} the variable{?s} {.var {vars_not_in_model}} named in {.arg {arg_name}} {cli::qty(vars_not_in_model)} {?is/are} not present in the original model")
  }
